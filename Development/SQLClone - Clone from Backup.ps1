...
ForEach ($db in $DeplyDbs)
    {
        ...
        Write-Host "Creating Snapshot for Database: $db at $(get-date) "
        ...
        $BckLatestDate = Get-ChildItem -Path $BckPath  | sort LastWriteTime | select -last 1
        $BckLatestSet = Get-ChildItem -Path $BckPath  | Where-Object {$_.LastWriteTime -gt ($BckLatestDate.LastWriteTime).AddSeconds(-2)} | Sort-Object Name 
        ...
        Write-Host "Creating Clone for database $db at $(get-date) "
